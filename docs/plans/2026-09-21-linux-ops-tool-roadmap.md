...
### 2026-09-27（补记十四）迭代 4b：Nginx 蓝绿的真实切流，跑通并验证

- **变更原因**：迭代 4（Nginx 蓝绿）的第二段：把「起新槽位 → 就绪 → 切流 → 观察 → 排空停旧槽位」
  真正实现出来，并在容器里用真 Nginx + 真 curl 验证。规格与记录见
  `docs/plans/2026-09-25-iteration-4.md` 第 15–17 节。
- **交付**：`ReleaseAdapter` 与 `RuntimeAdapter` 按槽位操作（端口加 `slot` 参数，空 = 单槽形态）；
  `NginxAdapter` 端口与实现（受管配置的渲染/解析、`nginx -T` 判「有没有被加载」、`nginx -t` 校验、
  原子替换 + `-s reload`，以及失败时的三种收尾）；`DeployService` 的蓝绿分支
  （`internal/application/deploy_slot.go`）；新增错误码 `NGINX_CONFIG_INVALID`（409/31）与
  `NGINX_RELOAD_FAILED`（409/32）；容器 harness 新增 `check_bluegreen`（27 项，镜像加 nginx 与 curl）。
- **最有分量的那条证据**：**curl 拿到的正文就是「现在服务的是哪一版」**——
  `version=1.0.0` → 部署 2.0.0 → `version=2.0.0` → 回滚 → `version=1.0.0`；
  而且**第二次部署期间持续打请求（每 50ms 一次）的失败数为 0**（成功 ≥ 50）。
  这是「发布期间服务不中断」这条路线图验收标准的直接证据。
- **四处按实测修正**（都已就地写进规格）：
  1. **`CheckLoaded` 必须在候选文件写到位之后做**：`nginx -T` 列的是它实际加载到的文件，
     而第一次部署时受管文件还不存在——先检查后写文件的话，第一次部署必然失败（容器第一次跑
     就撞上了）。现在顺序是「写候选 → 检查有没有被加载 → `nginx -t` → 原子替换 → reload」，
     前三步都在 reload 之前，任何一步失败都把原内容换回去、**流量一点没动**。
  2. 观察窗口**只采样就绪**（规格原本还写了 `NRestarts` 的增量）：端口上没有这个信息，
     为它扩端口会让每个实现各自解释「什么算一次重启」；而进程反复重启会让就绪探活跟着抖，
     抖动本身就会被抓住。
  3. `active` 的含义扩成「正在接流量」（4a 的第一个提交里就改了），不为蓝绿另立 `serving`。
  4. `applyDefaults` 不再给蓝绿规格补 `unitName`——那会让**同一份规格第二次校验**撞上
     「不得手写 unit 名」的互斥规则（生产里每次蓝绿部署都会因此失败）。
- **两条 harness 的教训**（都属于「断言看起来绿，其实没测到东西」）：
  - **`$( )` 里的 pass/fail 会丢计数**：`id=$(bg_deploy ...)` 是命令替换，子 shell 里的计数
    改动不会回到父 shell——日志里明明有一行 `FAIL`，收尾却报「0 项失败」。修法是不用命令替换
    取返回值，并加一道**元断言**：收尾时比对「计数」与「pass/fail 各自落的日志行数」，不一致
    直接判失败（那道元断言能抓住所有这一类的错误）。
  - **镜像里没有 `curl`**：切流全部成功而 curl 全失败，看起来像产品坏了。同时把「零失败」
    补成「失败数 = 0 **且** 成功数 ≥ 50」——「循环根本没跑起来」同样是零失败。
- **仍未验证**：真机（Linux 主机）上的蓝绿（容器证据不等于主机证据）、`app slot` 系列命令与
  reconcile（4c）、以及路线图要求的**压测与故障注入**（本迭代能证明的是「切流期间零失败请求」，
  不是压测结论）。
