...
- systemd timer 生成（属于 1c 的 Linux 适配；调度器与 timer 是两种可选机制，
  本迭代由 `opsd` 进程内的调度循环驱动）。
- 部署、备份、发布（属于后续迭代）。
...
1. **cron 解析依赖（需要用户确认）**：
   - 推荐引入 `github.com/robfig/cron/v3`——成熟的 5 字段 cron 解析，自带时区支持，
     测试覆盖完整。代价是把依赖从 3 个增加到 4 个，`AGENTS.md` 的依赖约定要同步更新。
   - 备选：自己实现 5 字段解析（约 200 行 + 测试）。不增加依赖，但日/月/周的
     语义细节、闰年、时区换算容易出错，风险显著更高。
   - 本项目**不**支持 `L`/`#`/`W`、秒字段和 `@every` 之外的别名，无论选哪种实现。
2. 调度器与 systemd timer 的关系：1b 用进程内循环，1c 会不会同时提供 timer 生成？
   若两者都做，必须明确「谁是权威」，否则同一计划可能被触发两次。
