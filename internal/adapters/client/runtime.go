...
func (c *Client) PrepareRuntime(ctx context.Context, appRef, slot string) (*v1.RuntimePrepareResponse, error) {
...
func (c *Client) RuntimeHealth(ctx context.Context, appRef, slot string) (*v1.RuntimeHealthResponse, error) {
	var out v1.RuntimeHealthResponse
	if err := c.do(ctx, http.MethodGet, runtimePath(appRef, "health")+runtimeSlotQuery(slot), nil, nil, &out); err != nil {
		return nil, err
	}
	return &out, nil
}
...
func (c *Client) runtimeAction(ctx context.Context, appRef, action string, in RuntimeActionInput) (*v1.Operation, bool, error) {
	...
	created, err := c.doWithStatus(ctx, http.MethodPost, runtimePath(appRef, action), nil,
		v1.RuntimeActionRequest{Slot: in.Slot, IdempotencyKey: in.IdempotencyKey, CreatedBy: in.CreatedBy, Retry: in.Retry}, &out)
	if err != nil {
...
// runtimeSlotQuery 把槽位拼成查询参数（health 是 GET，没有请求体可放）。
// 空槽位不拼参数：单槽应用不该在 URL 里多一个空值。
func runtimeSlotQuery(slot string) string {
	if slot == "" {
		return ""
	}
